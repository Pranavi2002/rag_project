// handles both deepeval and ragas synchronously
...
function App() {
  ...
  const [context, setContext] = useState([]); // <-- store retrieved context
  ...
  const handleAsk = async (q) => {
    ...
    try {
      ...
      // Poll for metrics & reasoning asynchronously
      const interval = setInterval(async () => {
        const metricsRes = await getMetrics(q);
        if (metricsRes.data.metrics || metricsRes.data.reasoning) {
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">AskMyDocs — RAG + DeepEval + RAGAS</h1>
      ...
      <div className="mb-6 p-4 border rounded-lg shadow-sm bg-gray-50">
        ...
        />
        {uploading && <p className="text-sm text-gray-600 mt-2">Uploading files...</p>}
